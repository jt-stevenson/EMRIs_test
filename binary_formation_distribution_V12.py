...
def compute_torque(args, disk, M, Mbh):
    ...
    if args.TT=="B16": 
        return Gamma_I_p10 * exp_decay ##+ Gamma_GW
    elif args.TT=="G23": 
...
def main():
    ...
    parser.add_argument('-fEdd', type=float, default=0.01)  # real number
...
################################################################################################
    ...
    sigma_T = 6.6524587e-29 #Thomson cross section, SI
    eta = 0.1 
    Mdot_Edd = (4*np.pi*ct.G*Mbh*mp) / (eta * sigma_T * ct.c) #SI
    Mdot = args.fEdd * Mdot_Edd

    if args.DT  == "SG":
        disk = pagn.SirkoAGN(Mbh=Mbh, alpha=alpha, Mdot=Mdot, eps=eta)
        Rmin = disk.Rmin
        Rmax = disk.Rmax
    elif args.DT  == "TQM":
...
################################################################################################
    if printing == True:
        ...
        dir_name = f"outputs_V11/{args.DT}/alpha_{args.a}/Mbh_{args.M_SMBH:.1f}/fEdd_{args.fEdd}"
